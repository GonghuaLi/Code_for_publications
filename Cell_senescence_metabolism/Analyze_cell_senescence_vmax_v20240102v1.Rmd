...
writetxt(metaRep,'./results/rep/replication_fluxes_meta_analysis.txt')
...
for(i in 1:length(thenames)){
  studyname = thenames[i]
  tmpdir = paste0('./results/rep/',studyname,'_vmax/WDA.cen.bootstrap.txt')
...
pheatmap::pheatmap(DAscore.rep[idx,],scale = 'none',cluster_rows = T,cluster_cols = T,fontsize_row = 9,fontsize_col = 9,
                ...
                file ='./results/rep/Figure_common_pathwayheatmap_ALL_filter.pdf',
                  height = 8,width = 7)
...
metaflux = file2frame('./results/rep/replication_fluxes_meta_analysis.txt')
...
load("./results/rep/BJ_GSE56293_1_vmax/keygenes_for_figure4_network.Rdata")
...
metasigflux = data.frame(ID = metaflux$ID[idx],stringsAsFactors = F,
                         ...
                         Pvalue = metaflux$MetaPvalue[idx],
                         FDR = as.vector(metaflux$MetaFDR[idx]))
load("./results/rep/BJ_GSE56293_1_vmax/keymetabolites_for_figure4_network.Rdata")
...
#bootstrapLmflux3 = HDFdataLM$bootstrapLmflux3
#bootstrapLmflux3.beta.age = bootstrapLmflux3$beta.age
#bootstrapLmflux3.p.age = bootstrapLmflux3$p.age
#HDFdataLM$bootstrapLmflux3 = c()
#save(file = './results/HDF/HDFdataLM.Rdata',list = c('HDFdataLM'))
#save(file = './results/HDF/bootstrapLmflux3.beta.age.Rdata',list = c('bootstrapLmflux3.beta.age'))
#save(file = './results/HDF/bootstrapLmflux3.p.age.Rdata',list = c('bootstrapLmflux3.p.age'))
load('./results/HDF/HDFdataLM.Rdata')
load('./results/HDF/bootstrapLmflux3.beta.age.Rdata')
load('./results/HDF/bootstrapLmflux3.p.age.Rdata')
HDFdataLM$bootstrapLmflux3 = list()
HDFdataLM$bootstrapLmflux3$beta.age = bootstrapLmflux3.beta.age
HDFdataLM$bootstrapLmflux3$p.age = bootstrapLmflux3.p.age
...
```{r}
HDFexpr = file2frame('./data/cpm_m6.txt',row.names =1)
...
pdf('./Figures/Figure6/Figure6H_FDR.pdf')
...
p = plotGseaTable(pathways[topPathways], tfc.mev, fgseaRes, 
              gseaParam=0.5,)
pdf(file = './Figures/Figure6/GSEA_CP_KEGG.pdf',width = 10,height = 12)
print(p)
dev.off()
...
pdf('./Figures/Figure6/gsea_kegg_plot.pdf',width = 12,height = 12)
...
pheatmap::pheatmap(tmpdata[idx,],scale = 'row',cluster_rows = T,cluster_cols = T,
                  ...
                  #color=colorRampPalette(c('#3B4992','gray95','#BB0021'))(9),
                       #colorRampPalette(c('#008280','gray95','#BB0021'))(11),
                color=colorRampPalette(c('#4DBBD5FF','gray95','#E64B35FF'))(40),
                file ='./Figures/Figure6/Fatty acid metabolism_heatmap.pdf',
                  height = 6,width = 6)
```
