...
prepross_GPMM <- function(outlist,clin){
  ...
  flux3.annote = GPMMdata$Recon3.annote
...
#
#DEgenes = DEGenes.simplified()

xx = file2frame('./data/GSE245661/GSE245661_series_matrix.txt')
...
DEgenes = DEGenes.simplified(log2(fpkm+1e-6),catagory = clin$`subject status:ch1` != 'healthy volunteer (HV)' )

writetxt_forGPMM(DEgenes,'./results/DEgenes.txt')
...
fluxout = rGPMM(expr = fpkm,expressionType = 'fpkm',standardization = F,tissueType = 'muscle',numCores = 4)
gpmmresult = prepross_GPMM(fluxout,clin)
save(list = c('gpmmresult'),file = './results/GSE245661_gpmmresult.Rdata')

status = one_line_flux_analysis(gpmmresult$flux3,
                                fluxCase = gpmmresult$flux3.log2[,
                                                                 gpmmresult$clin.v$`!Sample_characteristics_ch3` != 'subject status: healthy volunteer (HV)' &
                                                                   gpmmresult$clin.v$`!Sample_characteristics_ch2` == 'Sex: Female'],
                                fluxControl = gpmmresult$flux3.log2[,gpmmresult$clin.v$`!Sample_characteristics_ch3` == 'subject status: healthy volunteer (HV)' &
                                                                      gpmmresult$clin.v$`!Sample_characteristics_ch2` == 'Sex: Female'],
                                flux3.annote = gpmmresult$flux3.annote,
                                outdir = './results/GSE245661_femaleV1/')

status = one_line_flux_analysis(gpmmresult$flux3,
                                fluxCase = gpmmresult$flux3.log2[,
                                                                 gpmmresult$clin.v$`!Sample_characteristics_ch3` != 'subject status: healthy volunteer (HV)' &
                                                                   gpmmresult$clin.v$`!Sample_characteristics_ch2` == 'Sex: Male'],
                                fluxControl = gpmmresult$flux3.log2[,gpmmresult$clin.v$`!Sample_characteristics_ch3` == 'subject status: healthy volunteer (HV)' &
                                                                      gpmmresult$clin.v$`!Sample_characteristics_ch2` == 'Sex: Male'],
                                flux3.annote = gpmmresult$flux3.annote,
                                outdir = './results/GSE245661_maleV1/')
...
###
load('./results/GSE245661_gpmmresult.Rdata')
DEaa = DEGenes.simplified(gpmmresult$flux3.log2,catagory = gpmmresult$clin.v$`!Sample_characteristics_ch3` != 'subject status: healthy volunteer (HV)' )
writetxt_forGPMM(DEaa,'./tmpDE.txt')
