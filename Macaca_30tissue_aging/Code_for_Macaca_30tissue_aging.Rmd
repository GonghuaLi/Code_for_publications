...
if(!dir.exists('./results')) 
  dir.create('./results')
tmpdirs = c('data_overall',"data_statu","trajactory",
            "trajactory_mrna","trajactory_combine",
...
#just for output html mRNA tsne
print(p)
...
DEpro.tissues.MetaDE = metaGene(pro.tissues.v,pro.tissues.info)
...
DEmrna.tissues.lm = get_tissue_DEgenes_lm(mrna.tissues,mrna.tissues.info,
                                          tissue.systems = tissue.systems)
DEmrna.tissues.MetaDE  = metaGene(mrna.tissues,mrna.tissues.info)
...
DEmet.tissues.lm = get_tissue_DEgenes_lm(met.tissues,met.tissues.info,
                                         tissue.systems = tissue.systems)
DEmet.tissues.MetaDE = metaGene(met.tissues,met.tissues.info)
...
pdf(file = './results/data_overall/Figure1F_number_of_Aging_moleculars_prometv2_npg.pdf',
    width = 12,height =5.1)
...
beta_cutoff = 0.008 #abs(sort(Metamrna$MetaBeta)[nrow(Metamrna)*0.1]), 2/25
...
#just for plot visualization
grid.arrange(arrangeGrob(grobs = c(C2_compare$plots.up,C2_compare$plots.down),ncol = 3,heights = c(3.5,3.5),
...
```{r}
enbrks <- 1.5*c(-1,-0.8,-0.6,-0.4,-0.2,-0.1,-0.05,-0.02,0.02,0.05,0.1,0.2,0.4,0.6,0.8,1)
...
pp = ggplot(,aes(tissue_pca$x[,1],tissue_pca$x[,2],color = tmpType)) + 
   ...
   scale_color_manual(values = c('#E64B35FF','#3C5488FF','#B09C85'))+
   theme(legend.text = element_text(size=12,face = 'bold'))+
   labs(title = "Molecular promet trajectory PCA")
print(pp)
dev.off()
...
xplot.beta <- function(ratio_beta,xtissue,tissueClass){
    ...
    tmp =  ggplot(,aes(x = ratio_beta[[xtissue]]$beta))+theme_classic()+
      ...
      theme(axis.line = element_line(size = 1.2))+ xlim(c(-0.06,0.06))+
...
## 11.2 TED pathways in different types
