...
get_DEexpr.age <- function(expr.tissue,metadata.tissue,control = 1){
    ddtype = metadata.tissue$stage
    DEexpr.age = DEGenes.simplified(expr.tissue,catagory = ddtype ==4, 
                                    subset = (ddtype ==4 | ddtype ==control))
  return(DEexpr.age)
}
...
calc_specificity <- function(x){
    output = matrix(0,30,5)
    
    for(i in 1:30){
        output[i,1] = i
        output[i,2:5] = colSums(x > i)
    }
    return(output)
}
