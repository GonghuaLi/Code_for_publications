function [tissueModel,Rxns] = createTissueSpecificModel_recon2(model, ...
...
%tmp_GPR = load('GPRs_recon2_for_GIMME');
%parsedGPR = tmp_GPR.parsedGPR;
%corrRxn = tmp_GPR.corrRxn;
...
genstat = ismember(model.geneSymbol,locus(logical(genePresence)));
%[rxnState,core_rxns] = gene_stat2rxns_stat(model,genstat);
[rxnState,core_rxns] = get_rxns_expr_byrule(model,genstat);
ExpressedRxns = model.rxns(core_rxns);
is_expr_rxns = ismember(model.rxns,ExpressedRxns);
generelatedrxns = sum(abs(model.rxnGeneMat),2)>0;
UnExpressedRxns = model.rxns(~is_expr_rxns & generelatedrxns);
unknown = model.rxns(~is_expr_rxns & ~generelatedrxns);
%%
...
a=length(rxnExpressed)
unExpressed = unique(unExpressed);
b=length(unExpressed)
