...
%%
addrxns_fastcore = findRxnIDs(Recon3,[bloodrxns;'DM_atp_c_';'biomass_reaction']);
...
for k = (r-1)*120+1:min(N,r*120)
    genstat = ismember(Recon3.geneSymbol,expressionData.Locus(logical(expr(:,k))));
    [rxnState,core_rxns] = get_rxns_expr_byrule(Recon3,genstat);
    core_rxns = unique([core_rxns;addrxns_fastcore]);
    core_rxns = core_rxns(core_rxns > 0);
    outmodels{k} = fastcore(Recon3, core_rxns,1e-4);
    %outmodels{k}.lb = lb(:,k);
    %outmodels{k}.ub = ub(:,k);
