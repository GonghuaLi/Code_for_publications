function [lb,ub] =  get_lb_ub_rxns_by_PQM_from_constructed_models(parsfile)
%% read parsfile
...
for i = 1:N
     tname = ['./Mat_PQM_nofva/',modelnames{i},'.mat'];
     if ~exist(tname)
         warning('Not exist: %s',tname);
       continue;
     end
    load(['./Mat_PQM_nofva/',modelnames{i},'.mat']);
    idx = findRxnIDs(Recon3,outmodel.rxns);
    Recon3.IsExprPQM(Recon3.IsExprPQM) = 0;
    Recon3.IsExprPQM(idx) = 1;
